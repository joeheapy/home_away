...
export const createBookingAction = async (prevState: {
  ...
  const user = await getAuthUser();
  // Delete all the bookings that have not been paid for
...
export const fetchStats = async () => {
  ...
  const paidBookingsCount = await db.booking.count({
    where: {
      paymentStatus: true,
    },
  });

  return { usersCount, propertiesCount, bookingsCount, paidBookingsCount };
}
